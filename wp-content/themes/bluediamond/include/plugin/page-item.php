<?php
	...
	*/
	...
	function print_page_header( $header, $caption = '' ){
		echo '<div class="page-header-wrapper gdl-border-x bottom">';
		echo '<div class="page-header-top-gimmick"></div>';
	...
	function print_item_header( $header, $addition = '' ){
		if(!empty($header)){
	...
	function print_column_service($item_xml){
		...
		$column_service_button_link = html_entity_decode(find_xml_value($item_xml, 'button-link'));
		$column_service_button_text = html_entity_decode(find_xml_value($item_xml, 'button-text'));
		...
		if( !empty($column_service_button_link) ){
			echo '<a class="gdl-button" href="' . $column_service_button_link . '">';
			echo $column_service_button_text;
			echo '</a>';
		}
	...
	function print_stunning_text($item_xml){
		...
		$icon = find_xml_value($item_xml, 'icon');
		if( !empty($icon) ){
			$thumbnail = wp_get_attachment_image_src( $icon , 'full' );
			$alt_text = get_post_meta($icon , '_wp_attachment_image_alt', true);
			echo '<img class="stunning-text-icon" src="' . $thumbnail[0] .'" alt="'. $alt_text .'"/>';
		}
