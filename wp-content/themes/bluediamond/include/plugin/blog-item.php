	...
	*/
	...
	function print_blog_medium( $item_class, $item_size, $num_excerpt, $full_content ){
		...
		while( have_posts() ){
			...
			echo '<span class="blog-info-head">' . __('Posted By : ','gdl_front_end') . '</span>';
			...
			if( $full_content == "No" ){
				echo gdl_get_excerpt( $num_excerpt, '... ' );
				echo '<div class="clear"></div>';
				echo '<a class="blog-continue-reading gdl-button" href="' . get_permalink() . '"> ' . $translator_continue_reading . '</a>';				
			}else{
				the_content($translator_continue_reading);
			}
			echo '</div>';
